...
The cost of the multi-agent approach is roughly 2x a single-agent audit in raw LLM spend. But the single-agent approach produces reports that need heavy human triage. The multi-agent approach produces reports where the high-severity findings have working PoCs attached. The human reviewing it can focus on "is this PoC realistic?" rather than "is this finding real?"

That tradeoff is not close.
